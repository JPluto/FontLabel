...
- (BOOL)loadEncryptedFont:(NSString *)filename withEncryptionKey:(NSString*)encryptionKey {
	NSString *fontPath = [[NSBundle mainBundle] pathForResource:filename ofType:@"aes"];
	if (fontPath == nil) {
		fontPath = [[NSBundle mainBundle] pathForResource:filename ofType:nil];
	}
	if (fontPath == nil) {
		...
		NSString* leafname = [filename stringByAppendingFormat: @".aes" ]; 
		fontPath = [documentsDirectory stringByAppendingPathComponent:leafname];		
	}	
	if (fontPath == nil) return NO;
	
	NSURL *url = [NSURL fileURLWithPath:fontPath];
	if ([self loadEncryptedFontURL:url withEncryptionKey:encryptionKey]) {
		[urls setObject:url forKey:filename];
		return YES;
	}
	return NO;
...
- (BOOL)loadEncryptedFontURL:(NSURL *)url withEncryptionKey:(NSString*)encryptionKey {
	NSData *data = [NSData dataWithContentsOfURL:url];
	CGDataProviderRef fontDataProvider = CGDataProviderCreateWithCFData((CFMutableDataRef)[data AES256DecryptWithKey:encryptionKey]);	
	if (fontDataProvider == NULL) return NO;
	CGFontRef newFont = CGFontCreateWithDataProvider(fontDataProvider); 
	CGDataProviderRelease(fontDataProvider); 
	if (newFont == NULL) return NO;
...
- (void)saveFont:(NSString *)filename asEncryptedFontFileName:(NSString *)asFileName withEncryptionKey:(NSString*)encryptionKey  {
	NSString *fontPath = [[NSBundle mainBundle] pathForResource:filename ofType:@"ttf"];
	if (fontPath == nil) {
		fontPath = [[NSBundle mainBundle] pathForResource:filename ofType:nil];
	}
	if (fontPath != nil) { 
		NSURL *url = [NSURL fileURLWithPath:fontPath];
		[self saveFontURL:url asEncryptedFontFileName:asFileName withEncryptionKey:encryptionKey];
	}
}

- (void)saveFontURL:(NSURL *)url asEncryptedFontFileName:(NSString *)asFileName withEncryptionKey:(NSString*)encryptionKey {
	NSData *dataFrom = [NSData dataWithContentsOfURL:url];
	NSData *dataTo = [dataFrom AES256EncryptWithKey:encryptionKey];
	
	NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES); 
	NSString* documentsDirectory = [paths objectAtIndex:0];     
	NSString* leafname = [asFileName stringByAppendingFormat: @".aes" ]; 
	NSString* filenameStr = [documentsDirectory stringByAppendingPathComponent:leafname];
	
	[dataTo writeToFile:filenameStr atomically:YES];
}
